/*
 * Punkt-Schritte zaehlen, ohne Grafikkarte.
 *
 * Aufruf:  node werkzeuge/punktschritte.mjs
 *
 * Millisekunden haengen am Geraet, Punkt-Schritte nicht. Dieses Werkzeug baut
 * denselben Rechenweg wie der Schattierer in reinem JS nach und *zaehlt*,
 * statt zu messen - damit laesst sich jede Sparidee bewerten, ohne dass ein
 * Treiber dazwischenredet und ohne dass jemand seinen Rechner hergeben muss.
 *
 * Was dabei herauskam (Seepferdchental, Schrittzahlen aus dem Bericht der
 * RX 9070 XT vom 17.8.2026):
 *
 *   Tiefe   Schritte je Punkt   innen %   davon Arbeit in Innenpunkten
 *       2                 263      28 %                          75 %
 *       4                 155       0 %                           1 %
 *       6                 892      68 %                          81 %
 *       8                1247       0 %                           2 %
 ...
 *      16                8548       4 %                           6 %
 *
 * Daraus folgt die wichtigste Einsicht fuer jede Optimierung: Die teuren
 * flachen Zeilen (2 und 6) sind teuer wegen der *Innenpunkte*, die teuren
 * tiefen Zeilen (14 und 16) wegen der Punkte, die *spaet entkommen*. Das sind
 * zwei verschiedene Probleme, und sie brauchen zwei verschiedene Mittel.
 *
 * Was dieses Werkzeug ausgibt, ist die Bewertung von BLA (bilineare
 * Naeherung, Zhuoran 2021) - dem Nachfolger der Reihenentwicklung. Siehe
 * unten.
 */
...
function blaBauen(bahn, eps, cBetrag) {
  ...
  for (let n = 0; n < L; n++) {
    ...
    r[n] = Math.max(0, eps * Math.hypot(zr, zi) - cBetrag);
  }
...
console.log('Sorte der Abweichung (200x112 Punkte, Tiefe 16)\n');
